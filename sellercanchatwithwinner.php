...
{
	extract($_POST);
	$k="insert into  `tbl_chat` values(null,'$sid','$uid','$message',curdate())";
	insert($k);
	// alert('success');
        ...
        <div class="container-fluid header bg-white p-0">
            <div class="row g-0 align-items-center flex-column-reverse flex-md-row">
                <div class="col-md-6 p-5 mt-lg-5">
                    <h1 class="display-5 animated fadeIn mb-4"><span class="text-primary"></span></h1>
...
<?php  
$q="SELECT * FROM `tbl_chat`  where (sender_id='$sid' and receiver_id='$uid')  OR (sender_id='$uid' AND receiver_id='$sid')";
$data=select($q);
foreach ($data as $key ) 
{
	if ($key['sender_id']== $sid) 
	{ ?>
